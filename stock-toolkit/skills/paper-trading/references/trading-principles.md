...
> **完整分配建仓策略**：分批节奏（首批 30-40% / 第二批 30-40% / 第三批 20-30%）、建仓触发前提、失败保护、一次性 vs 分批的选择标准，详见 [stock-daily-analysis/references/trading-discipline.md](../../stock-daily-analysis/references/trading-discipline.md) 第 3.0 节"分配建仓策略"。本节仅给出资金量维度的粗略指引，正式执行建仓时以 trading-discipline.md 3.0 为准。
...
| 事件类型 | 触发信号 | 处理方式 | 类别 |
|---------|---------|---------|------|
| 分批止盈①（take_profit_1） | **收盘**浮盈 ≥ +30%（现价 ≥ 触发价，涨破方向） | 次一交易日卖 1/3 | hard |
| 分批止盈②（take_profit_2） | **收盘**浮盈 ≥ +50% | 次一交易日再卖 1/3 | hard |
| 保本锁 | 收盘浮盈 ≥ +15%（atr-sync 自动） | cost_protection 上移至成本，只升不降 | hard（并入成本保护） |
...
| 层 | 触发（收盘确认） | 处理方式 | 说明 |
|------|---------|---------|------|
| 保本锁 | 浮盈 ≥ +15% | 保护线锁至成本（只升不降） | 行为保险，剪"浮盈变亏损" |
| 分批止盈①② | 浮盈 ≥ +30% / +50% | 次一交易日各卖 1/3 | 双层审计回测 C 方案（20/20 LOO 折） |
| 余仓跟随 | 阶梯卖完后 | trailing_stop peak−2.5×ATR | **禁止收紧 2.0**（回测实锤杀肥尾） |
| 消息止盈 | 重大利好兑现且触发阶梯 | 按阶梯执行 | 消息证伪走纪律卖出 |
| 市场止盈 | 市场环境转弱 | 按大盘风控规则执行 | 系统性风险 |

> **关键原则**：上涨方向的机械卖出只认三件套阶梯，目标价仅作展示参考不设条件；涉及亏损和趋势破坏的事件，永远优先于止盈。

```bash
# 止盈阶梯触发后（check-triggers 报 take_profit breach，收盘确认）——次日执行：
ptrade2 sell "股票" --qty <持仓1/3> --note "止盈阶梯①（成本¥X→现价¥Y 浮盈+Z%）"
# 然后标记条件：ptrade2 conditions "股票" --action trigger --trigger-price <现价>

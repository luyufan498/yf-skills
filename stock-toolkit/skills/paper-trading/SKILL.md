...
- **建仓信号**（l3-scan 复核通过 / 组合审查通道判定通过 / 分析设点）：不直接 buy → `taskbus watchpoint add <股> --price <触发价> --code <代码> --note "非交易时段判定，开盘触发:<原因>"` → 开盘后心跳检测现价≤触发价 → 交易时段内执行
- **止损信号**（非交易时段价格触及止损位）：不直接 sell → 标记止损卖点（watchpoint）→ 开盘后第一时间触发执行
...
2. 成立 → **先判断交易时段**（`date +%H:%M`，交易时段=9:30-11:30/13:00-15:00）：交易时段内直接按"消息试探仓建仓执行"框架执行；**非交易时段 → 挂 watchpoint（触发价=当前价或分析价，note 注明"非交易时段判定，开盘触发"），开盘后由心跳价格触发执行**
...
都不命中 → 维持 L3 / 设价格点 / 移除（原有逻辑）
```

**执行注意**：
- 判定通过 → **先判断交易时段**（见顶部"交易时段约束"）：交易时段内**直接执行**（allocate 建段 + buy + 成本保护 + 移动止损 + CALENDAR 评估）；**非交易时段（审查 19:00 属非交易时段）→ 挂 watchpoint（触发价=判定价/现价，note "审查判定通过，开盘触发:<通道/理由>"），开盘后由心跳触发执行**。复用下方"消息试探仓建仓执行"框架
...
- 判定依据写审查摘要（哪条通道/为什么命中/为什么放弃），投递群时可审计

### 📏 L3 数量上限（2026-08-24 加入，观察池集中度控制）

**上限**：L3 观察窗**非锁定（pin=0）≤25 只**（可在 20-30 区间调整）。**锁定（pin=1）不计入上限**——它是保护名额，锁定的票是用户/系统明确保护的（曾过正式分析/长期价值标的），不应因数量上限被挤出，也不占用普通观察名额。**超限时按挤出优先级淘汰，不超限不主动挤**（避免误杀，让子弹飞）。
...
**超限执行**：统计 L3 总数 → 减去 pin=1 锁定数 → 仅非锁定部分 >25 才挤。按优先级 1→5 找可挤的（只挤非锁定）→ `ptrade2 watchlist remove <股> --reason "L3超限挤出: <优先级原因>"`；挤出后如仍超限继续挤，直到 非锁定 ≤25。执行记录写审查摘要（挤了谁/为什么）。

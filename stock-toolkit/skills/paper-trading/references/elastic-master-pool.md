...
> **同票双组**：accounts.grp（tech/news）路由，同名双账户并存；冲突进晨审人工裁决。

## 三、段生命周期

```
技术组：入池(L2 待命) → 建仓点触发 → allocate(占段+拨预算+建账户,升L1) → buy/sell
        → 空仓 → release(现值回 free + 段归档 + 7日冷却) → 池去向(flag 开=archived 终态 / 关=降 L2)
消息组：收编(G1-G4/TTL) → sleeve-open(sleeve_ledger 扣款+成员账户 grp=news+pending 单+开槽)
        → sleeve-fill(开盘价成交+挂三件套) → 保护链退出 → 槽 partial/closed（sleeve-close-slot 对账）
移交桥：sleeve-migrate（单向一次，原成本结转+双 ledger 对转+加仓锁，禁回迁）
```

- **allocate**：从 free 拨 budget → 建账户（SQLite 原子事务）。校验：free 足够、单股 ≤30%×total、总 open 段 <20、不在冷却期、无已 open 段。**初始段默认总池 5%**（策略矩阵见 trading-discipline 3.0.0）。
- **topup**：段内追加弹药（账户 total/available += amount，free -= amount）。校验累计 ≤30%×total；消息组账户禁 topup（闸门）。
- **release**：空仓后现值回 free → 段 closed + 7 日 cooldown。L1 需 manual。回落语义见 flag。
- **sleeve-open**：sleeve_ledger 扣款 → 成员账户（等权）→ pending 待成交单 → event_slots 开槽；G3 活跃槽并入不加坑、关闭槽二波=新键；坑上限 20（与主池 20 段位并存互不侵占）。
...
- 账户/操作/条件：`accounts`（v7+grp 列，UNIQUE(stock_name,grp) 支持同票双组）/ `operations`/`conditions`/`condition_history`/`exright_applied` 规范化表。

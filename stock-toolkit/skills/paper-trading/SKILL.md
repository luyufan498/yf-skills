...
**排序（软加分，只影响先看谁；2026-09-09 加入）**：同一批候选/复核对象排队时——**甜点区（m10 15~25%）+ 有跳水段（`ptrade2 check-plunge` depth≤-15%）** 优先；甜点区但无跳水段 降级（样本外 fwd10 -0.45%/中位 -2.79%/胜 39%）。**只改顺序，不改动作**：不构成入场门、不构成加仓档位依据、不产生资金路径；**消息组禁用**（宪法 2.6 技术面入场门）。依据：quant-factor-validation——样本外 241 只随机 A 股，有跳水段−无跳水 同日配对 fwd10 +1.68pp(t 2.79)、fwd20 衰减至 +1.21pp(t 1.3)；池内"中速最强/极深必避"两个细节参数**样本外未复现**，故不写入门控。

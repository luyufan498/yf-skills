...
**⚠️ 截断保护（2026-08-25 加入）**：若本轮组合审查发生**响应截断/输出不完整**（monitor 报 truncated、continuation 失败、或摘要缺失步骤），**禁止执行任何移除/挤出/出池操作**（`watchlist remove`、L3 超限挤出、僵尸剔除）——截断意味着 agent 未完整评估全部标的，移除有误杀风险（可能误伤有 watchpoint/CALENDAR 保护的票）。此时只做**非破坏性操作**（通道判定记录、设价格点、挂 CALENDAR、升级 L2），移除类操作**留到下一轮完整运行时再执行**，并在摘要注明"⚠️ 截断保护：移除操作延后"。

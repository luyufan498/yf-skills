...
def _normalize_stock_name(stock_name: str) -> str:
    ...
    if _opencc_converter is not None:
        return _opencc_converter.convert(stock_name)
    return stock_name
...
def _get_stock_name_suggestions(stock_name: str, manager: PortfolioManager) -> str:
    """当精确匹配失败时，查找相似的股票名称并给出建议"""
    accounts = manager.list_accounts()
    if not accounts:
        return ""

    matches = difflib.get_close_matches(stock_name, accounts, n=3, cutoff=0.4)
    if matches:

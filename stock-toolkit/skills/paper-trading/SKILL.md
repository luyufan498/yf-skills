...
**背景**：L3 观察窗不做每日深度分析（9:40 批量只覆盖 L1/L2），消息试探仓的判定（stock-daily-analysis 步骤 7）对 L3 是断的——L3 个股强消息入库后无人触发消息门。修复：**组合审查（19:00）每日基于当天全部事件统一做通道判定**，优于每条消息单独触发（效率高 + 能看消息组合效应）。
...
**执行注意**：
- 判定通过 → **直接执行**（allocate 建段 + buy + 成本保护 + 移动止损 + CALENDAR 评估），复用下方"消息试探仓建仓执行"框架

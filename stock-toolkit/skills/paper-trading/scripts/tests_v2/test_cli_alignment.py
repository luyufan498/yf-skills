...
def test_normalize_stock_name(ws):
    ...
    assert normalize_stock_name('英維克') == '英维克'
    assert normalize_stock_name('赛力斯') == '赛力斯'


def test_get_stock_name_suggestions(ws):
    from paper_trading_v2.helpers import get_stock_name_suggestions
    class FakeManager:
        def list_accounts(self):
            return ['赛力斯', '英维克', '中科曙光']
    m = FakeManager()
    # 相近名给出建议
    s = get_stock_name_suggestions('赛力', m)
    assert '赛力斯' in s
    # 空账户返回空串
    assert get_stock_name_suggestions('赛力', object()) == ''


def test_auto_exright_check_no_account_returns_false(ws):
    """get_account 返回 None 时 early-return False（不触网络/DB）"""
    from paper_trading_v2.helpers import auto_exright_check
    class FakeTrader:
        def get_account(self, stock_name):
            return None
    assert auto_exright_check(FakeTrader(), '赛力斯') is False
...
def test_info_command_markdown(ws):
    ...
    from paper_trading_v2.storage import SqlStorage
    from paper_trading_v2.models import Account, CapitalPool
    # code=None 让 auto_exright_check 走 early-return，避免真实网络除权抓取

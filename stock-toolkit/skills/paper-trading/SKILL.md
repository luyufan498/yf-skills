...
- **L2 信号缓冲**（`strategy='NEWS'`，存池表，无交易权限）：收编扫描/晨审按 G1-G4 清单闸落库——G1 数据卫生 / G2 次新否决（上市 <40 交易日硬拒绝，`watchlist-add` 对 NEWS 硬检查）/ G3 事件键归并（键=watchlog.event_key，活跃槽并入不加坑，关闭槽再遇同催化=二波新键）/ G4 TTL 2 交易日作废。**禁设价格点/禁 conditions/禁技术档位语言**；**msg-expiry-scan（2026-09-08）**：消息槽 T+5 论点失效扫描（晚审 0c 自动清退）——满 5 交易日+A(买入后最高<买价×1.03)+B(现价≤买价)+C(买入次日后无 imp4 新事件)→候选；池紧(free<40 万/槽≥15)5 日即清、池松放宽 T+8/10；触发当天事件排除不计 C；**G5 发酵度核验（2026-09-08 用户定稿，附 C2 收编闸非本缓冲闸）**：sleeve-open 前跑 `ptrade2 check-pulse`——🟢 照常；🟡/🔴（窗口拉升段+现价回踩/情绪顶）须 agent 核验消息时序后决定（不自动放行也不硬拦）：新消息独立增量（细则/订单/实锤，与已发酵主题不同源）→ 放行 note 留痕"时序核验：新消息"；滞后扩散（同主题余波）→ 不建槽上报；🔴 放行门槛更高需硬证据；晚审 0b 回检判定质量（软保护只报不拦——回踩规律统计：拉升段回撤>5% 后 5 日仅 25-31% 收复）
- **L1 事件槽**（`ptrade2 sleeve-open` 建槽，成员等权；**M3 已于 2026-09-02 开闸投用**，首批 3 槽灰度）：**每槽成员帽 `MAX_SLOT_MEMBERS=3`（9/2 用户裁决：一般 1-2 只即够，sleeve_open.py commit b9ad53e）**——开槽与 G3 并入两条路径都校验"活跃成员（exited_at 非 NULL 不占额）+ 新增 ≤3"，超出 ValueError 拒绝（不建槽不扣钱），**--force 不豁免**（策略硬帽，不同于同票双组冲突的人工放行）；超帽候选留 NEWS 缓冲或按新 event_key 另开独立槽。一事件多只是设计如此（等权稀释：预算整篮共享 ÷ 成员数，篮子总敞口天然封顶；"簇内选最强代表"已回测证伪——被丢成员均值 +2.9% > 代表 +2.2%）。每事件 1 槽、成员各建 NEWS 段（strategy='NEWS'，组由段 strategy 推导）、保护链三件套（sleeve-fill 开盘成交时挂载，与 atr-sync 同源常量）。晨审判定 → sleeve-open 建**待成交单（pending）**→ 心跳开盘后首扫（≥9:30）`sleeve-fill` 按当日开盘价统一成交（R7 防线：价≤0/偏离昨收>30%/ATR 解析失败 → 拒绝成交 + shadow_log fill_blocked 留痕，槽保持 pending 下轮重试）；TTL 内未成交（停牌等）→ `sleeve-cancel` 弃单进影子账#1。退出三条腿：①保护链自然退出 ②论点失效清退（msg-expiry-scan T+5，2026-09-08 起投用：晚审 0c 判定→自动卖+关槽，早期"灰度影子"设计退役）③移交桥 → 技术组。**槽占用 = event_slots.status∈(open,partial) 行数（20 坑）；全部成员清零 → 槽 closed 释放（`sleeve-close-slot` 对账归档）**
...
**运维命令**：`ptrade2 protect-orders-sync [股票名] [--dry-run] [--format json]` —— **只生成/刷新
...
**每日节拍**：`atr-sync` 收尾会**一并刷新止盈腿**（与保护线同一个每交易日首 tick 的节拍；价量没变=零写入）。
**运维命令**：`ptrade2 tp-orders-sync [股票名] [--dry-run] [--batch-id N] [--format json]`
= 手动按当前剩余仓位重算两档价并覆盖式重挂（**不重算成本基**；用于加仓/减仓后立即重挂或排障）。
...
③ 粘滞flag确认豁免：仅粘滞 flag（非破位）+ 大盘 m20≤3% + 非连亏 + 日线站上5日线且收阳
   ...
   → 退役：改走 G1-G4 清单闸 → sleeve-open
④ 动量仓（**趋势门已于 2026-09-09 整体删除**，只看动量甜点区 + 大盘）→ 常规升级技术组 L1 路径（甜点区措辞作废）
...
**排序（软加分，只影响先看谁；2026-09-09 加入，同日加"低点新鲜度"约束）**：同一批候选/复核对象排队时——**甜点区（m10 15~25%）+ 有跳水段（`ptrade2 check-plunge` depth≤-15%）+ 低点新鲜（`rdays` ≤20 交易日）** 优先；甜点区但无跳水段 降级（样本外 fwd10 -0.45%/中位 -2.79%/胜 39%）；**跳水段但低点距今 >20 交易日（`tag=stale`，"反弹陈旧"）不加分**——实证：甜点区内 低点近(≤20)−远(>20) 同日配对 fwd10 +1.25~1.62pp(t 1.75~2.20)、**fwd20 +3.88~4.29pp(t 3.50~3.88)**，2025/2026 分年皆成立（t 2.73 / 2.72），干净子集亦成立。**只改顺序，不改动作**：不构成入场门、不构成加仓档位依据、不产生资金路径；**消息组禁用**（宪法 2.6 技术面入场门）。依据：quant-factor-validation——样本外 241 只随机 A 股，有跳水段−无跳水 同日配对 fwd10 +1.68pp(t 2.79)、fwd20 衰减至 +1.21pp(t 1.3)；池内"中速最强/极深必避"两个细节参数**样本外未复现**，故不写入门控。
...
taskbus add CALENDAR "股票" --source message-position --priority 2 \
  --payload '{"due":"<10个交易日后>","event":"消息试探仓评估","check":"动量15-25%+强化条件(止损线过成本+事件有效)则段内加仓/升级正式仓,否则减仓或降观察"}'
```
...
- 升级正式仓需 动量 15~25% + **强化条件**（消息来源票 = 移动止损线已上移过成本 + 事件仍有效；**2026-09-09 修订：不再要求周趋势门通过**，见纪律文档 3.0.0.1 第 4 条）（段内加仓/topup 路径，见 3.4.3）
- **持有期统一状态机（2026-08-22 加入，见纪律文档 3.0.0.1）**：入场后通道使命结束——**动量维度只决定加不加仓**（甜点区→确认档15-20%；甜点区+强化条件→强化档至30%（消息来源票不查周趋势门，2026-09-09 修订，见纪律文档 3.0.0.1 第4条）；>25%不加仓；降回0~15%暂停加仓；转负不加仓），**价格维度只决定卖不卖**（成本保护/移动止损/利润梯度/目标价/证伪——见纪律文档 2.1-2.4）。**无动量止盈**（动量冲高≠卖出，移动止损锁盈）；**动量回落≠卖出**（保护位不破就持有观察）。
